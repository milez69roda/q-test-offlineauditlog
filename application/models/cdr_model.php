...
class Cdr_Model extends CI_Model {
    ...
    function get(){ 
		...
		$aColumns = array('review_date_start', 'kana_case_id', 'agent_id', 'audit_identifier', 'question1', 'question2', 'question3', 'question4', 'question5', 'score', 'comments', 'suggestion', 'feedback', 'audit_code' );
		...
		foreach( $rResult as $row ){
			...
			$rows[] = $row->feedback;    
			$rows[] = $row->audit_code;    
			$rows[] = '<a style="padding: 4px;" href="'.base_url().'cdr/?edit='.$row->id.'">Edit</a> | '.(($this->session->userdata('OFFAL_ISSUPER') == 1)?'<a style="padding: 4px;" href="javascript:void(0)" title="'.$row->feedback.'" onclick="COMMON.feedbackform(\'cdr\', '.$row->id.', this)">Feedback</a>':'');
