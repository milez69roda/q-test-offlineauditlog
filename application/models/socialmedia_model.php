...
class Socialmedia_Model extends CI_Model {
    ...
    function get(){ 
		...
		$aColumns = array('review_date_start', 'agent_id', 'audit_identifier', 'question1', 'question2', 'question3', 'question4', 'question5', 'question6', 'score', 'comments', 'suggestion', 'feedback', 'audit_code' );
		

		/* Indexed column (used for fast and accurate table cardinality) */
		$sIndexColumn = "social_media.id";
		...
		foreach( $rResult as $row ){
			...
			$rows[] = $row->feedback;    
			$rows[] = $row->audit_code;    
			//$rows[] = '<a href="'.base_url().'socialmedia/?edit='.$row->id.'">edit</a>';
			$rows[] = '<a style="padding: 4px;" href="'.base_url().'socialmedia/?edit='.$row->id.'">Edit</a> | '.(($this->session->userdata('OFFAL_ISSUPER') == 1)?'<a style="padding: 4px;" href="javascript:void(0)" title="'.$row->feedback.'" onclick="COMMON.feedbackform(\'social_media\', '.$row->id.', this)">Feedback</a>':'');
			$output['aaData'][] = $rows;
		}
